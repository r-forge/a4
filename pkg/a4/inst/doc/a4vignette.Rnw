...
<<LogisticRegressionPlotBars, fig=TRUE, term = FALSE>>=
...
<<plotLogRatioComplex>>=
  load(system.file("examples", "esetExampleTimeCourse.rda", package = "a4")
  ...
  plotLogRatio(e = logRatioEset, mx = 1, filename = "logRatioOverallTimeCourse.pdf", sep = ""),
      gene.fontsize = 8,
      orderBy = list(rows = "hclust", cols = NULL), colorsColumnsBy = c('compound'),
      within = "hours", shortvarnames = shortvarnames, exp.width = 1,
      main = paste("Differential Expression (trend at early time points) in", cl, "upon treatment with", compound),
