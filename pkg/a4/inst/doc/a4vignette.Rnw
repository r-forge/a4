...
<<plotComb2Samples, fig = TRUE, term = FALSE>>=
...
<<plotComb2SamplesWithAnnotation, fig = TRUE, term = FALSE>>=
...
<<plotCombMultipleSamples, fig = TRUE, term = FALSE>>=
...
it may be of value to look at the fold changes of several treatments (in columns)
for a selected list of genes (in rows).
 
<<GeneFClist, term = FALSE>>=
